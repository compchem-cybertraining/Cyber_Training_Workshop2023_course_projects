...
//  MATRIX q(*dyn_var.q);
  ...
  *dyn_var.p = *dyn_var.p + 0.5 * prms.dt * (*dyn_var.f);
  ...
  // Saving as old states to be used for FSSH_var
  vector<CMATRIX> dm_all_old(ntraj, CMATRIX(nadi, nadi));
  for(i=0; i<ntraj; i++){ dm_all_old[i] = *dyn_var.dm_adi[i]; }
...
//  dyn_var.update_density_matrix(prms, ham, 1); // This one is okay
  ...
  // FSSH, GFSH, MSSH, LZ, ZN, DISH
  else if(prms.tsh_method == 0 || prms.tsh_method == 10 || prms.tsh_method == 1 || prms.tsh_method == 2 || prms.tsh_method == 3
    ...
    // FSSH (0), FSSH_var (10), GFSH (1), MSSH (2), LZ(3), ZN (4)
    if(prms.tsh_method == 0 || prms.tsh_method == 10 || prms.tsh_method == 1 || prms.tsh_method == 2 || prms.tsh_method == 3  || prms.tsh_method == 4){

      /// Compute hop proposal probabilities from the active state of each trajectory to all other states 
      /// of that trajectory
      vector< vector<double> > g;
      g = hop_proposal_probabilities(prms, dyn_var, dm_all_old, ham, ham_aux);
