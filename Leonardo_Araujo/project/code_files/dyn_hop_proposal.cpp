...
//      cout<<"rho_ij = "<<denmat.get(i,j)<<"  Hvib_ij = "<<Hvib.get(i,j)<<endl;
  ...
  return g;
    ...
    MATRIX hopping_probabilities_fssh_var(dyn_control_params& prms, CMATRIX& Coeff, CMATRIX& Coeff_old, CMATRIX& Hvib){
/**
...
*/
        ...
        MATRIX g(nstates,nstates);
        CMATRIX denmat(nstates, nstates);
        CMATRIX denmat_old(nstates, nstates);

        denmat = Coeff * Coeff.H();

        denmat_old = Coeff_old * Coeff_old.H();
            ...
            for(j=0;j<nstates;j++){
                ...
                if(i!=j){
                    /**
                      ...
                      P(i->j) = ( rho_jj - rho_old_jj ) / rho_ii
...
vector<double> hopping_probabilities_fssh_var(dyn_control_params& prms, CMATRIX& denmat, CMATRIX& denmat_old, CMATRIX& Hvib, int act_state_indx){
/**
  DESCRIPTION TO BE UPDATED!!!
  \brief This function computes the surface hopping probabilities according to Tully's FSSH prescription.
  The surface-hopping probabilities may be Boltzmann-corrected
...
vector< vector<double> > hop_proposal_probabilities(dyn_control_params& prms, dyn_variables& dyn_var, vector<CMATRIX>& dm_all_old,
nHamiltonian& ham, nHamiltonian& ham_prev){
//vector<MATRIX>& prev_ham_dia){
/**
  This function computes the hop probabilities for each trajectory to hop from any state to all states
...
else if(prms.tsh_method == 10){ // FSSH_var

    //CMATRIX& dm_old = *dyn_var_old.dm_adi[traj];
    CMATRIX dm_old = dm_all_old[traj];

    g[traj] = hopping_probabilities_fssh_var(prms, dm, dm_old, Hvib, dyn_var.act_states[traj]);
