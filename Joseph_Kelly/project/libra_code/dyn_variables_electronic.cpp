    ...
    Args:
        ...
        params ( dictionary ): control parameters
            ...
            * **params["init_type"]** ( int ): the type of sampling of electronic DOFs
                ...
                - 4 : initialize all states according to "istate" using the multi-state 
                      MASH implementation (Runeson & Manolopoulos, arXiv:2305.08835)
            ...
            * **params["istate"]** ( int ): the index of the initial electronic state, used 
                only when **params["init_type"]** is 0 1 or 4, in which case it defines on 
...
*/
  ...
  for(int i=0;i<len(params.values());i++){
    ...
    else if(key=="nstates") { nstates = bp::extract<int>(params.values()[i]); }
  }
  ...
  for(traj=0; traj<ntraj; traj++){
    ...
    if(init_type==0){
      ...
      if(verbosity > 0){
        cout<<"======= Initialization type is "<<init_type<<" ========\n";
        cout<<"setting representation "<<rep<<" coefficients C_i for all i to complex numbers such that MASH initial conditions satisfied \n";
      }
      
      double HN = 0.0;
      for(i=1; i<=nstates; i++){HN = HN + 1.0/i; }       
      double alphaN = (nstates-1)/(HN-1);
      double shift = (alphaN-1)/(nstates*alphaN);

      for(i=0; i<nstates; i++){
        ksi = rnd.uniform(0.0,1.0);
        complex<double> ampl(cos(2.0*M_PI*ksi), sin(2.0*M_PI*ksi) );
        if(i==istate){ampl = ampl*sqrt(1/alphaN+shift);}
        else{ampl = ampl*sqrt(shift);}
        if(rep==0){ ampl_dia->set(i, traj, ampl );  }
        else if(rep==1){ ampl_adi->set(i, traj, ampl);  }
      }
    } //init_type==4
