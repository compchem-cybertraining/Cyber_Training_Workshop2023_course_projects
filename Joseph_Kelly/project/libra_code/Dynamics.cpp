...
//  dyn_var.update_density_matrix(prms, ham, 1); // This one is okay
  ...
  // FSSH, GFSH, MSSH, LZ, ZN, DISH, MASH
  else if(prms.tsh_method == 0 || prms.tsh_method == 1 || prms.tsh_method == 2 || prms.tsh_method == 3 
       || prms.tsh_method == 4 || prms.tsh_method ==5 || prms.tsh_method ==6){
    ...
    // FSSH (0), GFSH (1), MSSH (2), LZ(3), ZN (4), MASH (6)
    if(prms.tsh_method == 0 || prms.tsh_method == 1 || prms.tsh_method == 2 || prms.tsh_method == 3  || prms.tsh_method == 4 || prms.tsh_method ==6){

      /// Compute hop proposal probabilities from the active state of each trajectory to all other states 
      /// of that trajectory
      vector< vector<double> > g;
      vector<int> prop_states;
    
      // Propose new discrete states for all trajectories
      if(prms.tsh_method != 6){
        // hop proposals for FSSH, GFSH, MSSH, LZ, ZN
        g = hop_proposal_probabilities(prms, dyn_var, ham, ham_aux);
        prop_states = propose_hops(g, dyn_var.act_states, rnd);
      }
      else{
        // hop proposals for MASH
        prop_states = propose_hops_mash(old_states, *dyn_var.ampl_adi); 
      }
